...
Us = DisorderMPO([Us[1]])
# UsTaylor2 = random_transverse_field_ising_evolution(Js, hs, dτ; order=alg_trunc_disordermpo3)


βs = 1:0.5:5
# Evolve density matrix
function get_ξ(βs, Us, ps, alg_inversion)
    ξs = zeros(length(βs))
...
for (ix, tol) in enumerate(tols)
    alg = VOMPS_Inversion(1; tol = tol, maxiter = 100, verbosity = 2)
    ...
    scatter!(ax1,log.(βs).^2, ξs, label=L"f=%$(tol)",markersize = 16)
    lines!(ax1,log.(βs).^2, linmodel(log.(βs).^2,linparams), label=L"$p_2=%$(linparams[2])$")

    scatter!(ax2,dτ:dτ:βs[end],ϵs.+1e-16, label=L"f=%$(tol)",markersize = 16)

    scatter!(ax3, βs, times, label=L"f=%$(tol)",markersize = 16)

    scatter!(ax4,βs,DZs, label=L"f=%$(tol)",markersize = 16)
end

fig[1, 3] = Legend(fig, ax1, framevisible = false)
fig

save("convtol_traced.png",fig)

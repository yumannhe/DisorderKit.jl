...
alg_trunc_disordermpo = DisorderKit.QRTruncation(tol=1e-12, max_iter=20, trunc=truncbelow(1e-10), truncdim = truncdim(D_max))
nstep = 20
τf = nstep*dτ

βs = dτ:dτ:τf
# Evolve density matrix
function get_ξ(βs, Us, timer)
    ξs = zeros(length(βs))
    ...
    for (i,β) in enumerate(βs)
        ...
        alg_evolution = iDTEBD(alg_inversion, alg_trunc_Z, alg_trunc_disordermpo; invtol = invtol, nsteps = nsteps, verbosity = 2, truncfrequency = 1, inversion_frequency = inversion_frequency, timer_output = timer, max_inverse_dim = 2)
        ρs, ϵ = evolve_densitymatrix(Us, ps, alg_evolution, i; ρ0 = ρ0)
...
timer = TimerOutput()
ξs, Es, ϵs = get_ξ(βs, Us, timer)
TimerOutputs.print_timer(timer; allocations = false, compact = true)
